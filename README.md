...
1. 옥수박이 [개인 프로젝트][link-projects]를 진행하는 작업실이다.
    * '독서' 프로젝트 -- 책을 읽고 요약노트를 만들어 블로그에 공개한다.
    * '공부' 프로젝트 -- 중간/최종 보고서를 블로그에 공개한다.
    * '제작' 프로젝트 -- 중간/최종/사후 보고서와 작업물을 블로그에 공개한다.
    * '실행' 프로젝트 -- 비정기 보고서를 블로그에 공개한다.
2. 옥수박이 메모, 보고서, 잡글을 적는 [개인 블로그][link-archives]이다.
    * 블로그의 운영 목적은 생각을 정리해서 부족한 기억력을 보조하는 것이다.

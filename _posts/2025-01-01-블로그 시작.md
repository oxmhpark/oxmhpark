...
## 설계
- 포스트
  - [일지][link-blog-category-journal], [백업][link-blog-category-backup], [블로그 관리 기록][link-blog-category-maintenance]처럼 기록 자체가 목적인 게시물은 포스트로 규정하고 `_posts` 폴더 하위에 작성한다.[※1][link-repo-folder-posts]
  - 이미 작성한 포스트를 갱신하려는 경우에는 별도의 포스트로 분리해서 그 맥락과 이력을 추적할 수 있도록 한다.
- 페이지
  - [즐겨찾기][link-blog-page-bookmarks]나 [독서목표][link-blog-page-bookworks]처럼 내용과 형식을 활용하는 것이 목적인 게시물은 페이지로 규정하고 `_pages` 폴더 하위에 작성한다.[※2][link-repo-folder-pages]
...
- 카테고리 아카이브
  ...
  - 주의) 포괄적인 카테고리를 만들지 말 것; 단기적이고 구체적인 목표를 세우자.
- 태그 아카이브
...
[link-blog-page-bookmarks]: https://nemorium.net/bookmarks
[link-blog-page-bookworks]: https://nemorium.net/bookworks
